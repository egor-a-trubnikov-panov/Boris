...
import BlockEventEmitter from 'block-event-emitter';
...
class Block extends EventEmitter {
    ...
    constructor(domNode, options) {
        ...
        if (!this._DOM) {
            this._DOM = VDOM.getBJson(this.getDomNode())
        }
     ...
     * @returns {Block}
     */
    _unbindFrom(emitter, event, callback) {
        this._eventManager.unbindFrom(emitter, event, callback);
        return this;
    }

    _update(btrJSON) {
        this._DOM = VDOM.diff(this.getDomNode(), this._DOM, btr.setUniq(btr.processBtrJson(btrJSON)));
    }
    ...
    // Работа с состояниями и представлениями
     ...
     * @returns {Block}
     */
    _toggleState(stateName, stateVal1, stateVal2) {
     ...
     * @returns {Block}
     */
    _setElementState(domNode, stateName, stateVal) {
        if (domNode) {
            domNode = $(domNode);
     ...
     * @returns {Block}
     */
    _removeElementState(domNode, stateName) {
     ...
     * @returns {String}
     */
    _getElementState(domNode, stateName) {
        if (domNode) {
            domNode = $(domNode);
     ...
     * @returns {Block}
     */
    _toggleElementState(domNode, stateName, stateVal1, stateVal2) {
        stateVal1 = getStateValue(stateVal1);
        stateVal2 = getStateValue(stateVal2);
     ...
     */
    _findElement(elementName, parentElement) {
     ...
     * this._findAllElements('item').forEach(function (item) {
             ...
             * });
     */
    _findAllElements(elementName, parentElement) {
        parentElement = parentElement ? $(parentElement) : this.getDomNode();
        var
            view = this.getView(),
            elems = parentElement.find(
     ...
     * @returns {jQuery}
     */
    _createDomElement(params) {
        return $(btr.apply(extend({}, params, {
     ...
     */
    static fromDomNode(domNode, params) {
        if (!domNode) {
     ...
     * @returns {Block|null}
     */
    static initOnDomNode(domNode, params) {
        var initBlock;
        if (this._liveInit) {
    ...
    /**
     * Если для блока требуется отложенная живая инициализация,
     * следует перекрыть это свойство статическим методом.
     *
     * Этот код выполняется лишь однажды, при инициализации первого блока на странице.
     *
     ...
     */
    static _instantInitIf(condition) {
        if (!this._instantInitHandlers) {
     ...
     */
    static _liveBind(eventName, handler) {
        var blockClass = this;
     ...
     * @param {Function} handler
     */
    static _liveBindToElement(elementName, eventName, handler) {
        var blockClass = this;
        var blockName = this.getBlockName();
        var selectors = [
     ...
     * @returns {jQuery}
     */
    static _getLiveEventsScopeElement() {
     ...
     * @param {HTMLElement|jQuery} domNode
     */
    static destructOnDomNode(domNode) {
        domNode = $(domNode);
        var blockName = this.getBlockName();
     ...
     * @param {HTMLElement|jQuery|Block} domNode
     */
    static destructDomTree(domNode) {
        if (!domNode) {
     ...
     * @returns {Block}
     */
    static _getDomNodeFrom(domNode) {
        if (domNode) {
     ...
     * @param {jQuery} domNode
     */
    static _getDomNodeOptions(domNode) {
     ...
     * @returns {Object}
     */
    static _getDomNodeDataStorage(domNode, skipCreating) {
     ...
     * @returns {String}
     */
    static _getPropagationEventName(eventName) {
     ...
     * @type {String}
     */
    static get _delegateEventsCssClass() {
        return '_live-events'
    }
