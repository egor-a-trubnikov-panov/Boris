...
var __window__ = window, requestAnimationFrame = __window__.requestAnimationFrame
    || __window__.webkitRequestAnimationFrame
    || __window__.mozRequestAnimationFrame
    || __window__.msRequestAnimationFrame
    || function (cb) {
        return __window__.setTimeout(cb, 1000 / 60);
    };
 ...
 *
 * Добавление функции к указанному индексу в очереди кадров.
 *
 * @param  {Number}   index
 * @param  {Function} fn
 * @return {Function}
...
FastDom.prototype.schedule = function (index, fn) {
    var self = this;
    // Make sure this slot hasn't already been taken.
    // If it has, try re-scheduling for the next slot
    //
    // Убедитесь, что этот слот уже не были приняты.
    // Если это так, попытайтесь повторно планирование для следующего временного интервала
